...
public class MovieUI {
	...
	private JButton insertBt, updateBt, deleteBt, findBt
					, exitBt, sendBt;
	...
	private MouseAdapter listHandler = new MouseAdapter() {
		public void mouseClicked(java.awt.event.MouseEvent e) {
			String empno = (String)empTable.getValueAt(empTable.getSelectedRow(), 0);
			System.out.println(empno);
			empnoTf.setText(empno);
		};
	};
	public void findEmployee(){
		User  emp = model.search(empnoTf.getText());
		nameTf.setText(emp.getName());
		salaryTf.setText(""+emp.getSalary());
		if (emp instanceof Movie) {
			Movie mgr = (Movie) emp;
			addInfoTf.setText(mgr.getPosition());
			mgrCheck.setState(true);
			showType(MGR_STATE);
		}else if(emp instanceof Reservation) {
			Reservation eng = (Reservation) emp;
	...
	public void update() {
		...
		if(empCheck.getState()){
			model.update(new User(empno, ename, Integer.parseInt(salary)));
		}else if(mgrCheck.getState()){
			model.update(new Movie( empno,ename, Integer.parseInt(salary),addInfo));
		}else if(engCheck.getState()){
			model.update(new Reservation(empno, ename, Integer.parseInt(salary),addInfo));
		}
		showList();
		clear();
	}
	public void insert(){
		...
		String addInfo = addInfoTf.getText();
		if(empCheck.getState()){
			model.add(new User(empno, ename, Integer.parseInt(salary)));
		}else if(mgrCheck.getState()){
			model.add(new Movie( empno,ename, Integer.parseInt(salary),addInfo));
		}else if(engCheck.getState()){
			model.add(new Reservation(empno, ename, Integer.parseInt(salary),addInfo));
		}
		showList();
		clear();
	}
	...
	public MovieUI(){
		...
		exitBt = new JButton("종료");			sendBt = new JButton("전송");
		...
		main.setSize(900, 300);
		main.setVisible(true);
		main.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	}
	
	public void showList(){
		List<User> list = model.searchAll();
		if(list!=null){
